...
if __name__ == "__main__":
    if (len(sys.argv) != 4):
        print("Error: Usage is fontconvert.py (name of font) (name of output file) (name of SV file)")
        exit(-1)
    ...
    sv_file.write("localparam FONT_WIDTH = {};\n".format(width))
    sv_file.write("localparam FONT_HEIGHT = {};\n".format(height))

    chars_to_draw = (string.ascii_letters + " " + string.digits + string.punctuation)
    sv_file.write("localparam fontmap[256] = {")
    for idx in range(256):
        if (chr(idx) in chars_to_draw):
            sv_file.write(str(chars_to_draw.find(chr(idx))))
        else:
            sv_file.write("0")
        if (idx == 255):
            sv_file.write("};\n")
